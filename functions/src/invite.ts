...
async function sendInvite(a: any): Promise<boolean> {
  const origin = process.env.APP_ORIGIN || "https://bourbon.roamthrough.com";
  ...
  if (a.email) {
    jobs.push(
      sendEmail({
        to: a.email,
        subject: "You're invited to a bourbon tasting 🥃",
        html: `<div style="font-family:sans-serif;line-height:1.5">
          <h2 style="font-family:Georgia,serif">A tasting awaits</h2>
          <p>You've been invited to join a live bourbon tasting on Bourbon Roam.</p>
        ...
        text: `You're invited to a bourbon tasting. Join: ${url} (code ${a.joinCode})`,
      })
    );
  }
  if (a.phone) jobs.push(sendSms({ to: a.phone, body: `🥃 Join the bourbon tasting: ${url} (code ${a.joinCode})` }));
...
async function sendRecap(a: any): Promise<boolean> {
  const jobs: Promise<{ ok: boolean }>[] = [];
  if (a.email && a.html) {
    jobs.push(sendEmail({ to: a.email, subject: a.subject || "Your bourbon tasting recap 🥃", html: a.html, text: a.text }));
  }
