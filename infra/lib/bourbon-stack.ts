...
import { BourbonConfig } from "./config";

interface BourbonStackProps extends StackProps {
...
export class BourbonStack extends Stack {
  constructor(scope: Construct, id: string, props: BourbonStackProps) {
    ...
    const anthropicSecret = new secrets.Secret(this, "AnthropicKey", {
      description: "Bourbon Roam — Anthropic Workspace API key (set value after deploy)",
    });
    const anthropicAdminSecret = new secrets.Secret(this, "AnthropicAdminKey", {
      description: "Bourbon Roam — Anthropic Admin API key for org cost reporting",
    });
    const vapidSecret = new secrets.Secret(this, "VapidKey", {
      description: "Bourbon Roam — Web Push VAPID {publicKey, privateKey, subject} (set value after deploy)",
    });
    // Provider creds — swappable by updating the secret value (no redeploy).
    const resendSecret = new secrets.Secret(this, "ResendKey", {
      description: "Bourbon Roam — Resend {apiKey, fromEmail}. Shareable now; swap value to switch accounts.",
    });
    const twilioSecret = new secrets.Secret(this, "TwilioKey", {
      description: "Bourbon Roam — Twilio {accountSid, authToken, fromNumber}. Swap value to switch accounts.",
    });
    ...
    if (config.resendCognitoEmails) {
      emailKey = new kms.Key(this, "CognitoEmailKey", {
        description: "Bourbon Roam Cognito custom email sender",
    ...
    const googleOAuthSecret = new secrets.Secret(this, "GoogleOAuthSecret", {
      description: "Bourbon Roam — Google OAuth client secret: set { \"clientSecret\": \"...\" }",
    });
    const appleOAuthSecret = new secrets.Secret(this, "AppleOAuthSecret", {
      description: "Bourbon Roam — Apple Sign in private key (.p8): set { \"privateKey\": \"...\" }",
    });
    ...
    const apiWaf = new wafv2.CfnWebACL(this, "ApiWaf", {
      scope: "REGIONAL",
      defaultAction: { allow: {} },
      visibilityConfig: { cloudWatchMetricsEnabled: true, metricName: "bourbonApi", sampledRequestsEnabled: true },
      rules: [
        {
          name: "ApiRateLimit",
          priority: 1,
    ...
    // -------------------------------------------------------- Monitoring
    const alarmTopic = new sns.Topic(this, "Alarms", { displayName: "Bourbon Roam alarms" });

    // App alarms use an "bourbon-" name prefix so the admin dashboard can list only
    ...
    errAlarm("ApiErrors", "bourbon-api-errors", apiFn, 5, 5);
    errAlarm("EnrichErrors", "bourbon-enrich-errors", enrichFn, 3, 15);
    errAlarm("ReminderErrors", "bourbon-reminder-errors", reminderFn, 2, 15);
    errAlarm("ChatErrors", "bourbon-chat-errors", chatFn, 5, 15);

    new cw.Alarm(this, "DynamoThrottles", {
      alarmName: "bourbon-dynamodb-throttles",
