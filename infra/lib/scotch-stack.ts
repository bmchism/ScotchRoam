...
import { ScotchConfig } from "./config";

interface ScotchStackProps extends StackProps {
  config: ScotchConfig;
}
...
export class ScotchStack extends Stack {
  constructor(scope: Construct, id: string, props: ScotchStackProps) {
    ...
    const anthropicSecret = new secrets.Secret(this, "AnthropicKey", {
      description: "Scotch Roam — Anthropic Workspace API key (set value after deploy)",
    });
    const anthropicAdminSecret = new secrets.Secret(this, "AnthropicAdminKey", {
      description: "Scotch Roam — Anthropic Admin API key for org cost reporting",
    });
    const vapidSecret = new secrets.Secret(this, "VapidKey", {
      description: "Scotch Roam — Web Push VAPID {publicKey, privateKey, subject} (set value after deploy)",
    });
    // Provider creds — swappable by updating the secret value (no redeploy).
    const resendSecret = new secrets.Secret(this, "ResendKey", {
      description: "Scotch Roam — Resend {apiKey, fromEmail}. Shareable now; swap value to switch accounts.",
    });
    const twilioSecret = new secrets.Secret(this, "TwilioKey", {
      description: "Scotch Roam — Twilio {accountSid, authToken, fromNumber}. Swap value to switch accounts.",
    });
    ...
    if (config.resendCognitoEmails) {
      emailKey = new kms.Key(this, "CognitoEmailKey", {
        description: "Scotch Roam Cognito custom email sender",
    ...
    const googleOAuthSecret = new secrets.Secret(this, "GoogleOAuthSecret", {
      description: "Scotch Roam — Google OAuth client secret: set { \"clientSecret\": \"...\" }",
    });
    const appleOAuthSecret = new secrets.Secret(this, "AppleOAuthSecret", {
      description: "Scotch Roam — Apple Sign in private key (.p8): set { \"privateKey\": \"...\" }",
    });
    ...
    const apiWaf = new wafv2.CfnWebACL(this, "ApiWaf", {
      scope: "REGIONAL",
      defaultAction: { allow: {} },
      visibilityConfig: { cloudWatchMetricsEnabled: true, metricName: "scotchApi", sampledRequestsEnabled: true },
      rules: [
        {
          name: "ApiRateLimit",
          priority: 1,
    ...
    // -------------------------------------------------------- Monitoring
    const alarmTopic = new sns.Topic(this, "Alarms", { displayName: "Scotch Roam alarms" });

    // App alarms use an "scotch-" name prefix so the admin dashboard can list only
    ...
    errAlarm("ApiErrors", "scotch-api-errors", apiFn, 5, 5);
    errAlarm("EnrichErrors", "scotch-enrich-errors", enrichFn, 3, 15);
    errAlarm("ReminderErrors", "scotch-reminder-errors", reminderFn, 2, 15);
    errAlarm("ChatErrors", "scotch-chat-errors", chatFn, 5, 15);

    new cw.Alarm(this, "DynamoThrottles", {
      alarmName: "scotch-dynamodb-throttles",
